...
guchar G_padChar = (gchar) 0; /* Default pad charcater */
...
{ \
    register guint z, l = s/4; \
    register Word *tp = (Word *) t; \
    \
    for(z = 0; z < l; ++z) tp[z] = g_htonl(tp[z]); \
}
	    ...
	    guchar	        *initVec)
{
    ...
    static guchar       desKey[8], desKeys[128];
    static Word 	*oldKey = (Word *) desKey,
         		*keys   = (Word *) desKeys;
    static DES_What 	oldWhat;
    static DES_Mode 	oldMode;
    guchar 	        b0[8], b1[8];           /* feedback blocks */
	...
	if ((what == DES_encrypt) || (mode == DES_cfb) || (mode == DES_ofb))
	{
	    gint z;
	    Word r;
	    Word *k = keys;
	...
	if ((what != oldWhat) && ((mode == DES_ecb) || (mode == DES_cbc)))
	{
    ...
    ** of the "auto-generated" versions used in DES-aux.c.
    */
	...
	switch ((gint) mode)
	{
	case ((gint) DES_ecb): goto _ECB_;
	case ((gint) DES_cbc): goto _CBC_encrypt_;
	case ((gint) DES_cfb): goto _CFB_encrypt_;
	case ((gint) DES_ofb): goto _OFB_;
        }
    }
    else
    {
	...
	{
	case ((gint) DES_ecb): goto _ECB_;
        case ((gint) DES_cbc): goto _CBC_decrypt_;
        case ((gint) DES_cfb): goto _CFB_decrypt_;
        case ((gint) DES_ofb): goto _OFB_;
	}
    }
...
{
    guint i, mask, bits, parityBit, parityErrors = 0;

    for(i = 0; i < size; ++i, ++ptr)
    {
	for(mask = 0x80, bits = 0; mask > 0x01; mask >>= 1)
	    if (((guint) *ptr) & mask) ++bits;

	parityBit = bits % 2 == (guint) parity ? 0 : 1;

	if ((((guint) *ptr) & 0x1) != parityBit) ++parityErrors;
    }
    return parityErrors;
}


static guchar weakKeys[18][8] =
...
guchar *DES_generate (const DES_Generate what)
{
    static guchar buffer[8];
    static gint flag = 0;
    guchar *bp;
    long mask = what == DES_key ? 0xfe : 0xff;

    /* Set up a seed - 42 is the answer ... */
    if (!flag)
    {
	g_random_set_seed((gint) (getpid() * 42) ^ (gint) time(NULL));
	flag = 1;
    }
    do
    {
	for(bp = buffer; bp <= &(buffer[7]); *bp++ = (guchar) (g_random_int() & mask));
        if (what == DES_key) DES_setParity(buffer, 8, DES_odd);

    } while(what == DES_key ? DES_checkWeakKeys(buffer) : 0);
