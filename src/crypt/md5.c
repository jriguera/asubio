...
/*
 * Encodes input (guint32) into output (guchar). Assumes len is a
 * multiple of 4.
 */
inline static void Encode (guchar *output, guint32 *input, guint len)
{
    guint i, j;
...
/*
 * Decodes input (guchar) into output (guint32). Assumes len is a
 * multiple of 4.
 */
inline static void Decode (guint32 *output, guchar *input, guint len)
{
    guint i, j;
 ...
 */
static void MD5Transform (guint32 state[4], guchar block[64])
{
...
void MD5Update (MD5_CTX *context, guchar *input, guint inputLen)
{
    ...
    if (inputLen >= partLen)
    {
	memcpy((guchar *) & context->buffer[index], (guchar *) input, partLen);
    ...
    if ((inputLen - i) != 0)
    {
	memcpy((guchar *) & context->buffer[index], (guchar *) & input[i], inputLen - i);
    }
}
...
void MD5Final (guchar digest[16], MD5_CTX *context)
{
    ...
    /* Zeroize sensitive information. */
    memset((guchar *) context, 0, sizeof(*context));
}
